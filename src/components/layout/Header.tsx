      ...
      <div className="mx-auto flex max-w-6xl items-center justify-between px-4 py-4">
        ...
        <details className="relative md:hidden">
          ...
          <div className="absolute right-0 mt-3 w-72 rounded-2xl border border-slate-200 bg-white p-3 shadow-soft">
            <Link href="/calculators" className="flex min-h-11 items-center rounded-xl px-3 text-sm hover:bg-slate-50">주거비 계산기</Link>
            <Link href="/guides" className="flex min-h-11 items-center rounded-xl px-3 text-sm hover:bg-slate-50">주거비 가이드</Link>
            <Link href="/about" className="flex min-h-11 items-center rounded-xl px-3 text-sm hover:bg-slate-50">사이트 소개</Link>
            <Link href="/contact" className="flex min-h-11 items-center rounded-xl px-3 text-sm hover:bg-slate-50">문의</Link>
            <div className="mt-2 border-t border-slate-100 pt-2">
              {featuredCalculators.map((calculator) => (
                <Link key={calculator.slug} href={calculator.path} className="flex min-h-11 items-center rounded-xl px-3 text-sm text-slate-600 hover:bg-slate-50">
