...
import { calculateJeonseLoan } from "@/lib/calculators/loan";
    ...
    >
      <form onSubmit={handleSubmit(onSubmit)} className="rounded-3xl border border-slate-200 bg-white p-6 shadow-soft">
        <div className="grid gap-5 md:grid-cols-2">
          ...
          />
          <Controller name="guaranteeFeeRate" control={control} render={({ field }) => <PercentInput label="보증료율" value={field.value ?? 0} onChange={field.onChange} helper="선택 입력" min={0} max={10} />} />
          <Controller name="prepaymentFeeRate" control={control} render={({ field }) => <PercentInput label="중도상환수수료율" value={field.value ?? 0} onChange={field.onChange} helper="선택 입력" min={0} max={10} />} />
        </div>
